...
export default class App {
  ...
  constructor(rootDir) {
    if (!rootDir) {
      throw new Error('A root directory containing source files is required');
    }
    if (!FS.exists(rootDir)) {
      throw new Error(`The specified directory - ${rootDir} - does not exist or is not accessible`);
    }
    dotenv.config();
  ...
  async loadFiles() {
    const allFiles = FS.recurseDir(this.__rootDir, 'js');
